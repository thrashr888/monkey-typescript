...
// get the slice of a string
// string_slice('abcdef', 1, 3) => 'bc'
export default new Builtin(function(env: Environment, ...args: OObject[]): OObject {
  ...
  if (args[0].Type() !== STRING_OBJ || args[1].Type() !== INTEGER_OBJ || args[2].Type() !== INTEGER_OBJ) {
    return newError(
      'arguments to `string_slice` must be STRING,INTEGER,INTEGER, got %s,%s,%s',
      args[0].Type(),
      args[1].Type(),
      args[2].Type()
    );
  ...
  return new OString(arg1.slice(arg2, arg3));
});
